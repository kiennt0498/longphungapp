...
public class CongViecController {
    ...
    @MessageMapping("/getJobsXacNhan")
    public void getJobsXacNhan(@Payload String id) {
        id = cleanId(id);
        messagingTemplate.convertAndSend("/topic/jobsxacnhan/" + id, service.findByNhanVien_IdAndTrangThai(id, TrangThai.CHO_XAC_NHAN));
    }
    ...
    public void duyetSP(NhanViecReq req) {
        ...
        donHangService.setDonHang(id,TrangThai.CHO_XAC_NHAN, true);
//        service.createFistJobs(id);

        String maNVTK = service.getNhanVienTK(id, false);
        String maNVTao = service.getMaNVTao(id,false);

        tbService.thongBaoDonChoDuyet(maNVTao);
        tbService.thongBaoDonChot(maNVTao);
    ...
    public void nop(@Payload NhanViecReq req) {
        ...
        String maNVTao = service.getMaNVTao(req.getId(),true);
...
//        messagingTemplate.convertAndSend("/topic/jobsxacnhan/" + nv, service.findByNhanVien_IdAndTrangThai(nv, TrangThai.CHO_XAC_NHAN));
        messagingTemplate.convertAndSend("/topic/jobshoanthanhtk/" + nv, service.findByNhanVien_IdAndTrangThai(nv, TrangThai.DA_GIAO));
    }
}
